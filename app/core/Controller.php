...
class Controller {
    ...
    private $index, $method, $xmlPath;

    function __construct() {
        require_once                '../dev/engine/engine.php';
        require_once                '../dev/data/xmlData.php';
        $this->root                 = '../public/'; 
  ...
  function makeLink($str) {
	...
	if(preg_match_all($reg_exUrl, $str, $urls)) {
		...
		for($i=0; $i<$numOfUrlsToReplace; $i++) {
			$str = str_replace($urlsToReplace[$i], "<a href=\"".$urlsToReplace[$i]."\" target=\"blank\" class=\"LinkButton\">GO</a> ", $str);
		}
		return $str;
	} else {
		return $str;
